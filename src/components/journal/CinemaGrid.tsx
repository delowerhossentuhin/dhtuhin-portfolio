...
export function CinemaGrid({ entries }: { entries: CinemaEntry[] }) {
  ...
  const visible = useMemo(() => {
    ...
    switch (filter) {
      case 'movies':
        list = list.filter((e) => e.category === 'Movie');
        break;
      case 'tv':
        list = list.filter((e) => e.category === 'TV Series');
        break;
      case 'watchlist':
        list = list.filter((e) => e.status === 'Watchlist');
        break;
      case 'highest':
        list = list
      ...
      case 'recent':
        list = list
          ...
          .sort((a, b) =>
            (a.watchDate ?? '') < (b.watchDate ?? '') ? 1 : -1,
          );
        break;
    }
    if (genre !== 'All') {
      list = list.filter((e) => e.genres.includes(genre));
    }
    return list;
  ...
  const stats = useMemo(() => {
    const watched = entries.filter(
      (e) => e.status === 'Watched' || e.status === 'Rewatched',
    );
    const avg = watched.length
      ? watched.reduce((s, e) => s + e.rating, 0) / watched.length
      : 0;
    return {
            ...
            {filterDefs.map((f) => {
              ...
              return (
                <button
                  ...
                  className={`relative flex-none rounded-full px-3.5 py-1.5 text-xs uppercase tracking-[0.12em] transition ${
                    active ? 'text-ink-950' : 'text-ink-300 hover:text-white'
                  }`}
                >
                  {active && (
                    <motion.span
                      layoutId="cinema-filter-active"
            ...
            {visible.map((e, i) => (
              ...
              >
                ...
                <div
                  ...
                  style={{
                    background: `linear-gradient(160deg, ${e.posterColor} 0%, #0a0f1f 75%, #060b1a 100%)`,
                  }}
                />
                ...
                <div className="absolute left-2.5 right-2.5 top-2.5 flex items-center justify-between gap-2">
                  ...
                  {e.status === 'Watchlist' && (
                    <span className="flex items-center gap-1 rounded-full bg-amber-300/90 px-2 py-0.5 text-[9px] uppercase tracking-[0.16em] text-ink-950 backdrop-blur-sm">
                      <Bookmark size={9} /> Saved
                    </span>
                  )}
                  {e.status === 'Watching' && (
                    <span className="flex items-center gap-1 rounded-full bg-emerald-400/90 px-2 py-0.5 text-[9px] uppercase tracking-[0.16em] text-ink-950 backdrop-blur-sm">
                      <Eye size={9} /> Now
                    </span>
                  )}
                  {e.status === 'Rewatched' && (
                    <span className="flex items-center gap-1 rounded-full bg-sky-300/90 px-2 py-0.5 text-[9px] uppercase tracking-[0.16em] text-ink-950 backdrop-blur-sm">
                      <RotateCw size={9} /> Re
                    </span>
                  )}
                </div>
                ...
                <div className="absolute inset-0 grid place-items-center bg-black/70 px-4 opacity-0 transition group-hover:opacity-100">
                  <p className="line-clamp-6 text-center text-[11px] leading-relaxed text-ink-100">
                    {e.review}
                  </p>
                </div>
              </motion.button>
          ...
          >
            ...
            >
              ...
              <div
                ...
                style={{
                  background: `linear-gradient(180deg, ${open.posterColor} 0%, #060b1a 100%)`,
                }}
              >
              ...
              <div className="p-7 sm:p-9">
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <span className="rounded-full bg-white/5 px-2.5 py-0.5 text-[10px] uppercase tracking-[0.16em] text-ink-200">
                    {open.status}
                  </span>
                ...
                <h3 className="mt-4 h-display text-3xl text-white sm:text-4xl">
                  {open.title}
                </h3>
                ...
                <div className="mt-3 flex items-center gap-4">
                  ...
                  {open.watchDate && (
                    <span className="flex items-center gap-1 text-xs text-ink-300">
                      <Calendar size={11} /> {formatDate(open.watchDate)}
                    </span>
                  )}
                </div>
                ...
                <p className="mt-6 text-sm leading-relaxed text-ink-100">
                  {open.review}
                </p>
                ...
                {open.quote && (
                  <blockquote className="mt-6 border-l-2 border-amber-300/60 pl-4">
                    <div className="flex items-start gap-2 text-ink-200">
                      <Quote
                        size={14}
                        className="mt-1 flex-none text-amber-300/70"
                      />
...
function CinemaStat({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-xl border border-white/5 bg-white/[0.02] p-4">
      ...
      <p className="mt-1 font-mono text-[10px] uppercase tracking-[0.18em] text-amber-300/70">
        {label}
      </p>
    </div>
  );
...
function StarRow({
  ...
  return (
    ...
    >
      {Array.from({ length: 5 }).map((_, i) => {
        ...
        return (
          <span key={i} className="relative inline-block">
            <Star
              size={size}
              className={isFilled ? 'fill-amber-300' : ''}
              strokeWidth={1.6}
      ...
      {large && (
        <span className="ml-2 font-mono text-xs text-ink-200">
          {rating.toFixed(1)}
        </span>
      )}
    </div>
