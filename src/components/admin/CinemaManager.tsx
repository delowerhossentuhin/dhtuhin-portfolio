...
const ALL_GENRES = [
  'Action', 'Adventure', 'Animation', 'Biopic', 'Comedy', 'Crime',
  'Dark Comedy', 'Documentary', 'Drama', 'Epic', 'Fantasy', 'Historical',
  'Horror', 'Mystery', 'Noir', 'Romance', 'Sci-Fi', 'Thriller', 'Western',
];
...
export function CinemaManager() {
  ...
  function toggleGenre(genre: string) {
    if (!editing) return;
    const genres = editing.genres.includes(genre)
      ? editing.genres.filter((g) => g !== genre)
      : [...editing.genres, genre];
    setEditing({ ...editing, genres });
  }
            ...
            {items.map((c) => {
              ...
              return (
                <li key={c._id ?? c.id} className="flex items-center gap-4 p-4">
                  <div className="h-12 w-9 flex-none overflow-hidden rounded-md">
                    {c.posterUrl ? (
                      <img src={c.posterUrl} alt={c.title} className="h-full w-full object-cover" />
                    ) : (
                      <div
                        className="h-full w-full"
                        style={{ background: `linear-gradient(160deg, ${c.posterColor ?? '#1e293b'}, #060b1a)` }}
                      />
                    )}
                  </div>
                  <div className="min-w-0 flex-1">
                    <p className="truncate font-display text-sm text-white">
                      {c.title}{' '}
                      <span className="font-mono text-[10px] text-amber-300/70">{c.year}</span>
                    </p>
                    <p className="mt-0.5 truncate text-[11px] text-ink-400">
                      {c.category} · {c.status}{!isDb && ' · seed'}
                    </p>
                  </div>
                  <span className="flex flex-none items-center gap-1 text-amber-300">
                    <Star size={11} fill="currentColor" strokeWidth={0} />
                    <span className="font-mono text-xs text-ink-200">{c.rating.toFixed(1)}</span>
                  </span>
                  <div className="flex flex-none gap-1.5">
                    <button
                      onClick={() => setEditing({ ...c, genres: Array.from(c.genres ?? []), watchDate: toIsoDate(c.watchDate) })}
          ...
          >
            ...
            >
              <div className="mb-5 flex items-center justify-between">
                <h2 className="font-display text-lg text-white">{editing._id ? 'Edit entry' : 'New entry'}</h2>
              ...
              <div className="space-y-4">
                ...
                <div className="grid gap-4 sm:grid-cols-[2fr_1fr]">
                  <Field label="Title">
                    <input className="cm-input" value={editing.title} onChange={(e) => setEditing({ ...editing, title: e.target.value })} />
                  </Field>
                  <Field label="Year">
                    <input type="number" className="cm-input" value={editing.year} onChange={(e) => setEditing({ ...editing, year: Number(e.target.value) })} />
                  </Field>
                </div>
                ...
                <div className="grid gap-4 sm:grid-cols-3">
                  <Field label="Category">
                    <select className="cm-input" value={editing.category} onChange={(e) => setEditing({ ...editing, category: e.target.value as CRecord['category'] })}>
                      <option value="Movie">Movie</option>
                      <option value="TV Series">TV Series</option>
                    </select>
                  </Field>
                  <Field label="Status">
                    <select className="cm-input" value={editing.status} onChange={(e) => setEditing({ ...editing, status: e.target.value as Status })}>
                  ...
                  <Field label="Rating (0–5)">
                    <input type="number" min={0} max={5} step={0.5} className="cm-input" value={editing.rating} onChange={(e) => setEditing({ ...editing, rating: Number(e.target.value) })} />
                  </Field>
                </div>
                ...
                {/* Watch date + Poster color */}
                <div className="grid gap-4 sm:grid-cols-2">
                  <Field label="Watch date">
                    <input
                      type="date" className="cm-input"
                      value={editing.watchDate ? (typeof editing.watchDate === 'string' ? editing.watchDate.slice(0, 10) : '') : ''}
                      onChange={(e) => setEditing({ ...editing, watchDate: e.target.value || null })}
                    />
                  </Field>
                  <Field label="Poster color (fallback)">
                    <input
                      type="color"
                      className="h-10 w-full cursor-pointer rounded-md border border-white/10 bg-transparent"
                      value={editing.posterColor ?? '#1e293b'}
                      onChange={(e) => setEditing({ ...editing, posterColor: e.target.value })}
                    />
                  </Field>
                </div>

                {/* Poster URL */}
                <Field label="Poster URL (paste image link from TMDB or IMDB)">
                  <input
                    className="cm-input"
                ...
                {editing.posterUrl && (
                  <div className="flex items-center gap-3">
                    ...
                    />
                    <p className="text-xs text-ink-400">Poster preview</p>
                  </div>
                )}

                {/* Review */}
                <Field label="Review">
                  <textarea className="cm-input min-h-[100px]" value={editing.review ?? ''} onChange={(e) => setEditing({ ...editing, review: e.target.value })} />
                </Field>

                {/* Quote */}
                <Field label="Quote">
                  <input className="cm-input" value={editing.quote ?? ''} onChange={(e) => setEditing({ ...editing, quote: e.target.value })} />
                </Field>
              ...
              <div className="mt-6 flex justify-end gap-2">
                <button onClick={() => setEditing(null)} className="rounded-full border border-white/10 bg-white/[0.02] px-4 py-2 text-sm text-ink-200">
                  Cancel
                </button>
                <button
