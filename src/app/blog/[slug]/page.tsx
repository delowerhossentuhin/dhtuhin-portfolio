...
export async function generateStaticParams() {
  return seedBlogs.map((p) => ({ slug: p.slug }));
}
...
export async function generateMetadata({
  ...
  const post = seedBlogs.find((p) => p.slug === params.slug);
    ...
    <article>
      ...
      <header className="relative overflow-hidden border-b border-white/5">
        ...
        <div className="container-tight py-20 sm:py-28">
          <Link
            href="/blog"
            className="inline-flex items-center gap-1.5 text-xs text-ink-300 transition hover:text-white"
          >
            <ArrowLeft size={12} /> Back to all posts
          </Link>

          <p className="mt-8 font-mono text-xs uppercase tracking-[0.18em] text-sky-300">
            {post.category}
          </p>
          <h1 className="mt-3 h-display text-4xl text-white sm:text-5xl md:text-6xl">
            {post.title}
          </h1>
      ...
      <div className="container-tight py-16 sm:py-20">
        ...
        <div className="mt-12 flex flex-wrap gap-2 border-t border-white/5 pt-8">
          {post.tags.map((t) => (
            <span
              key={t}
              className="rounded-full border border-white/10 bg-white/[0.02] px-3 py-1 text-xs text-ink-300"
            >
              #{t}
            </span>
          ))}
        ...
        <div className="mt-12 flex items-start gap-4 rounded-2xl border border-white/5 bg-white/[0.02] p-6">
          <div className="grid h-12 w-12 flex-none place-items-center rounded-full bg-gradient-to-br from-sky-400/30 to-azure-700/30 font-display text-lg text-white">
            T
          </div>
          <div>
            ...
            <p className="mt-1 text-sm text-ink-300">
              Final-year CSE undergrad at AIUB. Research interests in federated learning,
              medical AI, and interpretable models. Always reachable.
            </p>
            <Link
              href="/contact"
            ...
            <div className="mt-6 grid gap-5 md:grid-cols-2">
              {related.map((r) => (
                ...
                >
                  ...
                  />
                  <h4 className="font-display text-lg text-white transition group-hover:text-sky-200">
                    {r.title}
                  </h4>
          ...
          <div className="rounded-3xl border border-white/5 bg-gradient-to-br from-sky-500/[0.05] via-white/[0.02] to-azure-700/[0.05] p-8 sm:p-12">
            <div className="grid items-center gap-8 md:grid-cols-[1.2fr_1fr]">
              <div>
                ...
                <h3 className="mt-3 h-display text-2xl text-white sm:text-3xl">
                  Get the next essay when it lands.
                </h3>
                <p className="mt-2 text-sm text-ink-300">
                  No marketing. One email per post. Unsubscribe anytime.
                </p>
              </div>
              <NewsletterForm />
            </div>
